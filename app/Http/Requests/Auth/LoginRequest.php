...
class LoginRequest extends FormRequest
{
    ...
    public function authenticate(): void
    {
        ...
        // Jika login pakai username, pastikan ada di DB
        if ($loginType === 'username' && ! \App\Models\User::where('username', $this->input('login'))->exists()) {
        ...
        if (! Auth::attempt([$loginType => $this->input('login'), 'password' => $this->input('password')], $this->boolean('remember'))) {
            ...
            throw ValidationException::withMessages([
                'login' => 'Username atau email tidak ditemukan atau password salah.',
            ]);
        }
