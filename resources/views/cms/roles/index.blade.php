...
@section('content')
    <main class="flex-grow p-4 sm:p-6 lg:p-8">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-slate-500 md:ms-2 dark:text-slate-400">List roles</span>
                    </div>
                </li>
            </ol>
        </nav>
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                        @forelse ($roles as $key => $role)
                            <tr
                                ...
                                <td scope="row" class="px-6 py-4 font-medium text-slate-900 dark:text-white">
                                    {{ $role->name }}
                                </td>
                                <td class="px-6 py-4 flex items-center space-x-3">
                                    ...
                                    <a href="{{ route('roles.show', $role->id) }}"
                                        class="text-green-600 hover:text-green-800 dark:text-green-400" title="Detail">
                                        <!-- Icon Eye -->
        ...
        @include('cms.roles.modal-delete')
    </main>
@endsection
...
@push('js')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const deleteRoleTitle = document.getElementById('deleteRoleTitle');

            // Buka modal saat tombol delete ditekan
            deleteButtons.forEach(btn => {
                btn.addEventListener('click', () => {
                    const roleId = btn.getAttribute('data-id');
                    const roleTitle = btn.getAttribute('data-title');

                    deleteForm.action = `/cms/roles/${roleId}`;
                    deleteRoleTitle.textContent = roleTitle;
