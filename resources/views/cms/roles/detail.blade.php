...
@section('content')
    <main class="flex-grow p-4 sm:p-6 lg:p-8">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-slate-500 md:ms-2 dark:text-slate-400">Role Detail</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Role Detail</h2>
        </div>

        <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md h-fit w-full">
            <h3 class="text-xl font-semibold text-slate-800 dark:text-white mb-6">Role Detail</h3>
            <form id="permission-form">
                <div class="mb-6">
                    <label for="name" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Role
                        Name</label>
                    <input type="text" id="name" name="name"
                        ...
                        placeholder="Masukkan nama role" value="{{ $role->name }}" readonly>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                    ...
                    @forelse ($authorities as $manageName => $permissions)
                        <div
                            ...
                            <h4 class="text-lg font-semibold text-slate-800 dark:text-white mb-3">{{ $manageName }}</h4>
                            <div class="space-y-2">
                ...
                <div class="flex justify-end space-x-2">
                    <button type="button" onclick="window.location.href='{{ route('roles.index') }}'"
                        class="px-4 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-md hover:bg-slate-300 dark:hover:bg-slate-600">Back</button>
                </div>
            </form>
        </div>
    </main>
