...
@section('content')
    <main class="flex-grow p-4 sm:p-6 lg:p-8">
        ...
        @include('cms.pages.modal-delete')
    </main>
@endsection
...
@push('js')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const deletePageTitle = document.getElementById('deletePageTitle');

            // Buka modal saat tombol delete ditekan
            deleteButtons.forEach(btn => {
                btn.addEventListener('click', () => {
                    const pageId = btn.getAttribute('data-id');
                    const pageTitle = btn.getAttribute('data-title');

                    deleteForm.action = `/cms/pages/${pageId}`;
                    deletePageTitle.textContent = pageTitle;
